...
Output a JSON object with: needs_clarification (bool), reason (string or null), questions (array)."""
    ...
    def plan_with_clarifications(
        ...
        This is the main entry point for planning. It merges any clarifications
        into the prompt before calling the planning logic.
        ...
        Raises:
            ValueError: If the LLM response cannot be parsed
        """
        # Merge clarifications into the prompt if provided
        ...
        response = self.llm.generate(
            prompt=f"User prompt: {enriched_prompt}",
            system=self.system_prompt,
            temperature=1.0,
        )
